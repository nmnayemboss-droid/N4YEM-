...
<html lang="bn">
<head>
    <title> NM NAYEM - Ultra Hosting Panel</title>
    <meta charset="utf-8">
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .edit { background: #58a6ff; color: #000; } /* এডিট বাটন কালার */
        ...
        /* --- ফাইল ম্যানেজার এবং এডিটর স্টাইল --- */
...
<div class="container">
    ...
    <div class="branding">
        <h1 class="nayem-text">NAYEN</h1>
        <p style="font-size: 11px; color: #8b949e; letter-spacing: 3px;">ছিঁড়তে না পারলে টানবেন না এতে আপনার সময় নষ্ট হবে..!¡</p>
    </div>
</div>
...
<div class="social-bar">
    ...
    <a href="https://www.instagram.com/nayemsheikh2026?igsh=OXU3NDhyNjM4ZXk0" target="_blank" class="social-icon"><i class="fa-brands fa-instagram"></i></a>
...
<script>
        ...
        fetch('/list_files/' + activeProj).then(res => res.json()).then(data => {
            ...
            data.files.forEach(f => {
                html += `<div class="file-item">
                            <span onclick="loadFile('${f}')" style="cursor:pointer; color:var(--accent); font-size:13px;">📄 ${f}</span>
    ...
    function deleteFile(f) {
        Swal.fire({ title: 'মুছে ফেলবেন?', text: `${f} স্থায়ীভাবে ডিলিট হবে!`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ff4d4d', background: '#0a0a0a', color: '#fff' })
        .then(res => {
            if(res.isConfirmed) {
                fetch('/delete_file', {
                    method: 'POST',
